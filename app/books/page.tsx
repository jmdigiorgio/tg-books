      ...
      {Object.entries(booksBySeries).map(([series, seriesBooks]) => (
        <div key={series} className="mb-16">
          <div className="flex flex-col gap-3 mb-12">
            <h2 className={`${emilysCandy.className} text-4xl text-purple-600 relative`}>
              {series}
              <div className="absolute bottom-0 left-0 w-32 h-1 bg-purple-400/50 -mb-2"></div>
            </h2>
            <div className="flex gap-2">
              {getSeriesGenres(seriesBooks).map((genre) => (
                ...
                >
                  <span className="px-3 py-1 text-sm bg-purple-200 text-purple-900 rounded-full">
                    {genre}
                  </span>
                </Tooltip>
              ))}
